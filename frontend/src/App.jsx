...
const EU_MAP = {
  ...
  cyprus:'Cypriot','czech republic':'Czech',czechia:'Czech',denmark:'Danish',
  estonia:'Estonian',finland:'Finnish',france:'French',germany:'German',
  greece:'Greek',hungary:'Hungarian',ireland:'Irish',italy:'Italian',
  latvia:'Latvian',lithuania:'Lithuanian',luxembourg:'Luxembourgish',malta:'Maltese',
  netherlands:'Dutch',holland:'Dutch',poland:'Polish',portugal:'Portuguese',
  romania:'Romanian',slovakia:'Slovak',slovenia:'Slovenian',spain:'Spanish',
  sweden:'Swedish','united kingdom':'British',uk:'British',norway:'Norwegian',
  iceland:'Icelandic',switzerland:'Swiss',liechtenstein:'Liechtensteiner',
  austrian:'Austrian',belgian:'Belgian',bulgarian:'Bulgarian',croatian:'Croatian',
  cypriot:'Cypriot',czech:'Czech',danish:'Danish',estonian:'Estonian',
  finnish:'Finnish',french:'French',german:'German',greek:'Greek',
  hungarian:'Hungarian',irish:'Irish',italian:'Italian',latvian:'Latvian',
  lithuanian:'Lithuanian',luxembourgish:'Luxembourgish',maltese:'Maltese',
  dutch:'Dutch',polish:'Polish',portuguese:'Portuguese',romanian:'Romanian',
  slovak:'Slovak',slovenian:'Slovenian',spanish:'Spanish',swedish:'Swedish',
  british:'British',norwegian:'Norwegian',icelandic:'Icelandic',swiss:'Swiss',
}
...
const NATIONALITY_SUGGESTIONS = [
  ...
  'Indian','Pakistani','Bangladeshi','Sri Lankan','Nepali','American','Canadian',
  'Australian','Chinese','Filipino','Nigerian','Kenyan','South African','Brazilian',
  'Mexican','Turkish','Egyptian','Moroccan','Algerian','Ukrainian','Russian',
  'Belarusian','Georgian','Armenian','Azerbaijani','Thai','Vietnamese','Indonesian',
  'Malaysian','Singaporean','Japanese','Korean',
]
...
// ─── Rate type detection ──────────────────────────────────────────────────────
// Returns 'gross' | 'net' | null
function detectRateType(raw) {
  if (!raw) return null
  const lower = raw.trim().toLowerCase()
  if (lower.startsWith('gross') || lower.startsWith('gros')) return 'gross'
  if (lower.startsWith('net') || lower.startsWith('nett')) return 'net'
  return null
}

// Net monthly → hourly: monthly ÷ 19.5 ÷ 0.43 ÷ 8
function calcHourlyFromMonthly(monthly) {
  if (!monthly || isNaN(monthly) || monthly <= 0) return null
  return monthly / 19.5 / 0.43 / 8
}

// Parse the buy rate input and build the formatted string + hourly
// Input examples: "gross 700 SEK/Hour", "net 25000 SEK/Month", "700 gross SEK", "netto 5000 EUR/Month"
function parseBuyRateInput(raw) {
  ...
  const nums = (numMatch || []).map(n => parseFloat(n.replace(/[\s,]/g, ''))).filter(n => !isNaN(n) && n > 0)
  const amount = nums[0]
  ...
  // Detect period
  const isMonthly = /month|mån/i.test(raw)

  if (!amount) return { type, formatted: raw, hourly: null, currency }

  if (type === 'gross') {
    // Gross is always hourly
    const formatted = `Gross - ${amount} ${currency}/Hour`
    return { type: 'gross', formatted, hourly: amount, currency }
  }

  if (type === 'net') {
    // Net is monthly; auto-calc hourly
    const monthly = amount
    const hourly = calcHourlyFromMonthly(monthly)
    const hourlyStr = hourly ? hourly.toFixed(2) : '?'
    const formatted = `Net - ${monthly} ${currency}/Month (i.e, ${hourlyStr} ${currency}/Hour)`
    return { type: 'net', formatted, monthly, hourly, currency }
  }
...
// isEU: true | false | null
// rateType: 'gross' | 'net' | null
// workAuth: string
function buildCandidateInfo(name, isEU, rateType, workAuth) {
  const n = name || '[Candidate Name]'

  // Last bullet depends on rate type
  // gross → "Has own Limited company to manage payroll in Sweden."
  // net   → "He/She is willing to work under Avance payroll."
  // null  → "Has own Limited company to manage payroll in Sweden." (default)
  const payrollBullet = rateType === 'net'
  ...
  // Insert work permit bullet for non-EU with work auth
...
const EMPTY_FORM = {
  ...
  requisitionName: '',
  billRate: 'Open',
  buyRateRaw: '',        // raw user input
  buyRate: '',           // formatted final value shown in email
  ...
  return (
    <div>
      <div style={{ position: 'relative' }}>
        <input
          ...
          style={{
            ...
            borderRadius: 'var(--radius)', padding: badgeLabel ? '9px 70px 9px 13px' : '9px 13px',
        ...
        {badgeLabel && (
          <span style={{
            ...
            background: badgeColor, color: '#fff', fontSize: 9.5, fontWeight: 800,
      ...
      {formattedValue && !focused && (
        <div style={{ marginTop: 5, fontSize: 11.5, color: rateType === 'gross' ? 'var(--success)' : 'var(--accent2)', animation: 'fadeIn 0.2s ease' }}>
          📧 Email will show: <strong>{formattedValue}</strong>
        </div>
      )}

      {/* Net hourly calc breakdown */}
      {rateType === 'net' && hourlyCalc && !focused && (
        <div style={{ marginTop: 5, padding: '7px 11px', background: 'rgba(59,130,246,0.07)', border: '1px solid rgba(59,130,246,0.2)', borderRadius: 7, fontSize: 11.5, color: 'var(--accent2)', animation: 'fadeIn 0.2s ease' }}>
      ...
      {rateType === 'gross' && !focused && formattedValue && (
        <div style={{ marginTop: 5, fontSize: 11, color: 'var(--text-dim)' }}>
          ✓ Gross rate — "Has own Limited company" shown in Candidate Info
        </div>
      )}
      {rateType === 'net' && !focused && formattedValue && (
        <div style={{ marginTop: 5, fontSize: 11, color: 'var(--text-dim)' }}>
          ✓ Net rate — "Willing to work under Avance payroll" shown in Candidate Info
        </div>
      )}
...
function NationalityInput({ value, onChange, onResolved }) {
  ...
  return (
    <div style={{ position: 'relative' }}>
      <div style={{ position: 'relative' }}>
        <input value={value} onChange={handleInput} onBlur={handleBlur} onKeyDown={handleKey}
          onFocus={e => { e.target.style.borderColor = 'var(--accent)'; if (value && suggestions.length > 0) setOpen(true) }}
          placeholder="Type country or nationality…" autoComplete="off"
          style={{ width: '100%', background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '9px 38px 9px 13px', color: 'var(--text)', fontFamily: 'var(--font-body)', fontSize: 13.5, outline: 'none', transition: 'border-color .2s', boxSizing: 'border-box' }}
        />
          ...
          {suggestions.map((s, i) => {
            ...
            return (
              <div key={s} onMouseDown={() => select(s)} onMouseEnter={() => setHighlighted(i)}
                style={{ padding: '9px 13px', cursor: 'pointer', fontSize: 13.5, background: i === highlighted ? 'var(--accent-glow)' : 'transparent', color: 'var(--text)', display: 'flex', alignItems: 'center', gap: 8, borderBottom: i < suggestions.length - 1 ? '1px solid var(--border)' : 'none', transition: 'background .1s' }}>
                <span>{isEuItem ? '🇪🇺' : '🌍'}</span>
                <span>{s}</span>
  ...
  return (
    <div style={{ animation: 'slideDown 0.3s ease', gridColumn: '1 / -1' }}>
      <div style={{ background: 'rgba(245,158,11,0.07)', border: '1px solid rgba(245,158,11,0.35)', borderRadius: 10, padding: '12px 14px' }}>
        ...
        <input value={value} onChange={onChange} placeholder="e.g. Dependent work permit, Valid till Dec 2029"
        ...
        />
        {value && <p style={{ fontSize: 11, color: 'var(--text-dim)', marginTop: 5 }}>📝 Bullet: "<em>{value} and his wife is working for Prodware as permanent employee on a sponsored visa.</em>"</p>}
      </div>
    </div>
  )
}
...
function CVUploader({ onExtracted, onTextReady }) {
  ...
  const process = async (file) => {
    ...
    if (!['pdf', 'docx', 'doc', 'txt'].includes(ext)) { setStatus('error'); setErr('Only PDF, DOCX, DOC or TXT supported.'); return }
    setStatus('loading'); setFileName(file.name); setErr(''); setPii(null)
    try {
      let raw = ext === 'txt' ? await file.text() : ext === 'pdf' ? await parsePDF(file) : await parseDOCX(file)
      if (!raw.trim()) { setStatus('error'); setErr('No text extracted. Try another format.'); return }
      const found = extractPII(raw)
      setPii(found); onExtracted(found); onTextReady(stripPII(raw)); setStatus('done')
  ...
  return (
    <div>
      <div onDrop={e => { e.preventDefault(); setDragOver(false); process(e.dataTransfer.files[0]) }}
        ...
        style={{ border: `2px dashed ${borderColor}`, borderRadius: 12, padding: 22, textAlign: 'center', cursor: 'pointer', background: dragOver ? 'rgba(59,130,246,0.05)' : status === 'done' ? 'rgba(16,185,129,0.04)' : 'var(--bg)', transition: 'all .25s', marginBottom: 10, transform: dragOver ? 'scale(1.01)' : 'scale(1)' }}>
      ...
      <Alert type="green">🔒 <strong>Privacy Safe:</strong> Email, Phone & DOB extracted in your browser only — <strong>never sent to Groq AI</strong>.</Alert>
      {pii && status === 'done' && (
...
export default function App() {
  ...
  const rebuild = (name, nat, rateType, workAuth) => {
    const eu = isEUNationality(nat)
    return buildCandidateInfo(name, eu, rateType, workAuth)
  }

  const setCandName = e => {
    const name = e.target.value
    setForm(f => ({
      ...f, candidateName: name, fullName: name,
  ...
  const handleNationalityResolved = (resolved) => {
    ...
    setForm(f => ({
      ...f, nationality: resolved, eligibility,
      workAuthorization: eu === true ? '' : f.workAuthorization,
      candidateInfo: candInfoManual ? f.candidateInfo : rebuild(f.candidateName, resolved, rateInfo.type, eu === true ? '' : f.workAuthorization),
    }))
  }
  ...
  const setCandInfo = e => { setCandInfoManual(true); setForm(f => ({ ...f, candidateInfo: e.target.value })) }

  const reset = () => {
    if (window.confirm('Reset all fields to defaults?')) { setForm(EMPTY_FORM); setCandInfoManual(false) }
  }
  ...
  const handleExtracted = (pii) => {
    setForm(f => {
      ...
      return {
        ...f, candidateName: name, fullName: name,
        emailId: pii.email || f.emailId, contactNumber: pii.phone || f.contactNumber,
        dob: pii.dob || f.dob, presentLocation: pii.location || f.presentLocation,
        nationality: resolvedNat, eligibility, cvSource: pii.cvSource || f.cvSource,
  ...
  const generateAI = async () => {
    ...
    try {
      ...
      const { data } = await axios.post(`${apiBase}/api/generate-sizzling`, { cv_text: form.cvText, jd_text: form.requirements })
      const [s1, s2, s3] = data.top_sentences
      setForm(f => ({ ...f, candidateName: f.candidateName || data.candidate_name || '', fullName: f.fullName || data.candidate_name || '', sizzlingLine1: s1 || '', sizzlingLine2: s2 || '', sizzlingLine3: s3 || '', sizzlingSkills: data.skills.join(', ') }))
      setAiSuccess(true)
    } catch (err) {
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
            ...
            <div>
              ...
              <Card title="Greeting & Role" icon="👋" delay={50}>
                <G2>
                  ...
                  <Field label="Candidate Name" hint="auto-fills on upload"><Inp value={form.candidateName} onChange={setCandName} placeholder="Ravi Teja" /></Field>
              ...
              <Card title="Submission Details" icon="📋" delay={100}>
                <G2>
                  ...
                  {/* ── Candidate Buy Rate — smart input ── */}
                  <div style={{ marginBottom: 13 }}>
                  ...
                  <Field label="Candidate Submitted"><Inp value={form.candidateSubmitted} onChange={set('candidateSubmitted')} placeholder="14th Apr 2025" /></Field>
                  <Field label="Submission Deadline"><Inp value={form.submissionDeadline} onChange={set('submissionDeadline')} placeholder="15th Apr 2025" /></Field>
              ...
              <Card title="Candidate Details" icon="🪪" tag="AUTO-FILL" delay={150}>
                <G2>
                  <Field label="Full Name" hint="auto-filled"><Inp value={form.fullName} onChange={set('fullName')} placeholder="Ravi Teja Pagadala" /></Field>
                  ...
                  <Field label="Contact Number" hint="🔒 local only"><Inp value={form.contactNumber} onChange={set('contactNumber')} placeholder="+46(0)7032191048" /></Field>
                  <div style={{ marginBottom: 13 }}>
                  ...
                  <Field label="Date of Birth" hint="🔒 local only"><Inp value={form.dob} onChange={set('dob')} placeholder="16th Sep 2002" /></Field>
            ...
            <div>
              ...
              <Card title="AI Sizzling Generator" icon="🤖" glow delay={50}>
                <Alert type="blue">🛡️ <strong>Privacy safe:</strong> Email, phone & DOB replaced with [REDACTED] before being sent to AI.</Alert>
                ...
                <button onClick={generateAI} disabled={aiLoading} style={{ width: '100%', padding: '12px', fontSize: 13.5, border: 'none', borderRadius: 9, background: aiLoading ? '#374151' : 'linear-gradient(135deg,#3b82f6,#6366f1)', color: '#fff', fontFamily: 'var(--font-body)', fontWeight: 700, cursor: aiLoading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, boxShadow: aiLoading ? 'none' : '0 4px 16px rgba(99,102,241,0.3)', transition: 'all 0.2s' }}>
                ...
                <div style={{ marginTop: 16 }}>
                  <Field label="Sizzling Line 1" hint="AI generated — editable"><TA value={form.sizzlingLine1} onChange={set('sizzlingLine1')} rows={3} glow={!!form.sizzlingLine1} placeholder="Experienced Test Manager with more than 20 years of experience coordinating and following up on testing activities between teams, deliveries and projects, including test planning and communication with stakeholders in agile environments using Scrum, Kanban and SAFE." /></Field>
                  <Field label="Skills" hint="comma separated — AI generated"><TA value={form.sizzlingSkills} onChange={set('sizzlingSkills')} rows={2} glow={!!form.sizzlingSkills} placeholder="Test Management, End to End Testing, System Testing, Jira, Zephyr, HP ALM, SAFE, Scrum, Kanban, SQL, REST-services, KPI Analysis, Integration Testing, Acceptance Testing, Regression Testing" /></Field>
                  <Field label="Sizzling Line 2" hint="AI generated — editable"><TA value={form.sizzlingLine2} onChange={set('sizzlingLine2')} rows={3} glow={!!form.sizzlingLine2} placeholder="Proficient in developing and analyzing KPIs and test coverage in projects, administering test cases in Jira, Zephyr, HP ALM and ReQTest, maintaining collaboration with vendors and Risk Management across large scale enterprise programs." /></Field>
                  <Field label="Sizzling Line 3" hint="AI generated — editable"><TA value={form.sizzlingLine3} onChange={set('sizzlingLine3')} rows={3} glow={!!form.sizzlingLine3} placeholder="Demonstrated hands-on testing of SQL databases, ETL flows, REST-services and complex integration-heavy systems within Banking, Retail and Healthcare sectors." /></Field>
                  <Field label="Additional Sizzling Bullets" hint="one per line — editable"><TA value={form.sizzlingExtra} onChange={set('sizzlingExtra')} rows={4} placeholder={"Fluent in English and Swedish both written and oral.\nAvailable immediately (Project ended).\nLives in Stockholm, Sweden and is flexible to be onsite 3 days a week."} /></Field>
                </div>
              </Card>
