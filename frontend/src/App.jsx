...
function parseBuyRateInput(raw) {
  if (!raw?.trim())
    return { type: null, formatted: "", hourly: null, currency: "SEK" };
  const lower = raw.trim().toLowerCase();
  let type = null;
  if (/\b(gross|gros)\b/.test(lower)) type = "gross";
  else if (/\b(net|nett|netto)\b/.test(lower)) type = "net";
  if (!type)
    return { type: null, formatted: raw, hourly: null, currency: "SEK" };
  ...
  if (type === "gross")
    return {
      type: "gross",
      formatted: `Gross - ${amount} ${currency}/Hour`,
      hourly: amount,
      currency,
    };
  if (type === "net") {
...
function buildHtmlEmail(f) {
  ...
  const detailLis = [
    ...
    f.eligibility &&
      li(`Eligibility to Work in job location: ${f.eligibility}`),
...
function parseApiError(err) {
  if (!err?.response) {
    ...
    // ERR_NETWORK or Network Error = backend not running
    return {
      type: "backend_down",
  ...
  if (status === 400)
    return {
      ...
      message: detail || "Check your CV text and job requirements.",
      action: null,
      retryable: false,
    };
  if (status === 401)
    return {
      ...
      message: "Your GROQ_API_KEY is invalid or expired. Check your .env file.",
      action: null,
      retryable: false,
    };
  if (status === 403)
    return {
      type: "auth",
      title: "Access Denied",
      message: detail || "Access denied by Groq API.",
      action: null,
      retryable: false,
    };
  if (status === 413)
    return {
      type: "payload",
      title: "CV Text Too Large",
      message:
        detail ||
        "The CV text is too large. Trim it to the most relevant sections.",
      action: null,
      retryable: false,
    };
  if (status === 422)
    return {
      ...
      message:
        detail ||
        "The AI could not process the text. Ensure CV and requirements are plain text.",
      action: null,
      retryable: false,
    };
  if (status === 429) {
...
function BackendBanner({ status }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1 }}>
        <span style={{ fontWeight: 700, fontSize: 12.5, color: "#f87171" }}>
          Backend Not Running —{" "}
        </span>
      ...
      <span
        style={{ fontSize: 11, color: "rgba(248,113,113,0.5)", flexShrink: 0 }}
      >
        port 8000
      </span>
    </div>
...
function Inp({ value, onChange, placeholder, disabled, highlight }) {
  return (
    <input
      ...
      onBlur={(e) =>
        (e.target.style.borderColor = highlight
          ? "rgba(16,185,129,0.4)"
          : "var(--border)")
      }
    />
      ...
      >
        ...
        <span
          style={{
            ...
            fontWeight: 700,
            fontSize: 13.5,
            color: glow ? "var(--accent2)" : "var(--text)",
            flex: 1,
          }}
        >
          {title}
  ...
  const badgeColor =
    rateType === "gross"
      ...
      : rateType === "net"
        ...
        : "var(--text-dim)";
  const badgeLabel =
    rateType === "gross" ? "GROSS" : rateType === "net" ? "NET" : null;
  return (
    <div>
      ...
      {formattedValue && !focused && (
        <div
          style={{
            marginTop: 5,
            fontSize: 11.5,
            color: rateType === "gross" ? "var(--success)" : "var(--accent2)",
          }}
        >
      ...
      {rateType === "net" && hourlyCalc && !focused && (
        ...
        >
          🔢 Calc: Monthly ÷ 19.5 ÷ 0.43 ÷ 8 ={" "}
          <strong>{hourlyCalc.toFixed(2)} /Hour</strong>
        </div>
      )}
      ...
      {rateType === "net" && !focused && formattedValue && (
        <div style={{ marginTop: 5, fontSize: 11, color: "var(--text-dim)" }}>
          ✓ Net rate — "Willing to work under Avance payroll" shown in Candidate
          Info
        </div>
      )}
      {!rawValue && (
        ...
        >
          💡 Start with{" "}
          <strong style={{ color: "var(--text-muted)" }}>gross</strong> (hourly)
          or <strong style={{ color: "var(--text-muted)" }}>net</strong>{" "}
          (monthly) — hourly auto-calculated
        </div>
      )}
    </div>
  );
...
function NationalityInput({ value, onChange, onResolved }) {
  ...
  const handleInput = (e) => {
    ...
    const filtered = NATIONALITY_SUGGESTIONS.filter((s) =>
  ...
  const select = (s) => {
    onResolved(resolveNationality(s));
    setSuggestions([]);
    setOpen(false);
  };
  const handleBlur = () => {
    setTimeout(() => {
      setOpen(false);
      if (value) {
        const r = resolveNationality(value);
        if (r !== value) onResolved(r);
      }
    }, 150);
  };
  const handleKey = (e) => {
            ...
            return (
              <div
                ...
                style={{
                  ...
                  fontSize: 13.5,
                  background:
                    i === highlighted ? "var(--accent-glow)" : "transparent",
                  color: "var(--text)",
                  display: "flex",
                  alignItems: "center",
                  gap: 8,
      ...
      {value && euStatus !== null && (
        <div
          style={{
            ...
            color: euStatus ? "var(--success)" : "#f59e0b",
          }}
        >
          {euStatus
            ? "✓ EU/EEA National — No work permit required"
    ...
    <div style={{ animation: "slideDown 0.3s ease", gridColumn: "1 / -1" }}>
      ...
      >
        ...
        >
          ...
          >
            auto-fills Candidate Info bullet
          </span>
        </label>
        <input
          value={value}
        ...
        {value && (
          <p style={{ fontSize: 11, color: "var(--text-dim)", marginTop: 5 }}>
            📝 Bullet: "
            <em>
              {value} and his wife is working for Prodware as permanent employee
              on a sponsored visa.
            </em>
            "
          </p>
    ...
    <div>
      ...
      <p style={{ fontSize: 10.5, color: "var(--text-dim)", marginTop: 4 }}>
        ...
        <em style={{ color: "var(--text-muted)" }}>
          Interview: {FIXED_INTERVIEW} {FIXED_JOIN_PREFIX}{" "}
          {value || "immediately."}
        </em>
      </p>
    </div>
...
function CVUploader({ onExtracted, onTextReady }) {
  ...
  return (
    <div>
      ...
      >
        ...
        {status === "done" && (
          <div style={{ animation: "fadeIn 0.3s ease" }}>
            ...
            <div style={{ fontSize: 12, color: "var(--text-muted)" }}>
              Parsed — PII auto-filled locally · AI gets safe copy
            </div>
            <div
      ...
      <Alert type="green">
        🔒 <strong>Privacy Safe:</strong> Email, Phone & DOB extracted in your
        browser only — <strong>never sent to AI</strong>.
      </Alert>
      {pii && status === "done" && (
        <div
          style={{
        ...
        >
          <strong style={{ color: "var(--text)", fontSize: 12 }}>
            🔍 Auto-detected (local only):
          </strong>
          <br />
          {pii.name && (
            <span style={{ marginRight: 12 }}>
              👤 <strong style={{ color: "var(--text)" }}>{pii.name}</strong>
            </span>
          )}
          {pii.email && (
            <span style={{ marginRight: 12 }}>
              ✉️ <strong style={{ color: "var(--text)" }}>{pii.email}</strong>
            </span>
          )}
          {pii.phone && (
            <span style={{ marginRight: 12 }}>
              📞 <strong style={{ color: "var(--text)" }}>{pii.phone}</strong>
            </span>
          )}
          {pii.dob && (
            <span>
              🎂 <strong style={{ color: "var(--text)" }}>{pii.dob}</strong>
            </span>
          )}
        </div>
      )}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    check();
    // re-check every 30 s so banner clears once they start the server
  ...
  const handleNationalityResolved = (resolved) => {
    ...
    const eligibility =
      eu === true ? "Yes, EU National" : eu === false ? "Yes" : "";
    setForm((f) => ({
      ...f,
  ...
  const handleWorkAuthChange = (e) => {
    ...
    setForm((f) => ({
      ...
      candidateInfo: candInfoManual
        ? f.candidateInfo
        : rebuild(f.candidateName, f.nationality, rateInfo.type, workAuth),
    }));
  };
  ...
  const handleExtracted = (pii) => {
    setForm((f) => {
      ...
      const resolvedNat = pii.nationality
        ? resolveNationality(pii.nationality)
        : f.nationality;
  ...
  const handleTextReady = (safeText) =>
    setForm((f) => ({ ...f, cvText: safeText }));
  ...
  const generateAI = async () => {
    if (!form.cvText.trim()) {
      setAiError({
        ...
        message: "Please upload a CV or paste CV text before generating.",
        retryable: false,
      });
      return;
    }
    if (!form.requirements.trim()) {
      setAiError({
        ...
        message: "Please fill in the Job Requirements field before generating.",
        retryable: false,
      });
      return;
    }
    ...
    try {
      ...
      if (!data?.top_sentences || !Array.isArray(data.top_sentences))
        throw Object.assign(new Error("Bad response"), {
          response: {
            status: 500,
            data: { detail: "Server returned unexpected format." },
          },
        });
      ...
      setForm((f) => ({
        ...
        sizzlingLine2: s2 || "",
        sizzlingLine3: s3 || "",
        sizzlingSkills: Array.isArray(data.skills)
          ? data.skills.join(", ")
          : data.skills || "",
      }));
      setAiSuccess(true);
      // mark backend as ok once we get a successful response
      setBackendStatus("ok");
    } catch (err) {
      const parsed = parseApiError(err);
      setAiError(parsed);
      if (parsed.type === "rateLimit" && parsed.waitSeconds)
        startRetryCountdown(parsed.waitSeconds);
    } finally {
      setAiLoading(false);
    }
  };
      ...
      >
        ...
        >
            ...
            <div>
              ...
              >
                <div style={{ fontSize: 9.5, color: "var(--text-muted)" }}>
                  AI-Powered · Privacy Safe · Europe Focus
                </div>
                {/* Backend status dot */}
                <span
                  title={
            ...
            {["form", "preview"].map((v) => (
              <button
                ...
                style={{
                  ...
                  transition: "all .2s",
                  boxShadow:
                    view === v ? "0 2px 8px rgba(59,130,246,0.3)" : "none",
                }}
              >
                {v === "form" ? "📝 Form" : "👁 Preview"}
              </button>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button
              onClick={handleCopy}
              style={copied ? successBtn : primaryBtn}
            >
          ...
          >
            ...
            <div>
              ...
              <Card title="Greeting & Role" icon="👋" delay={50}>
                <G2>
                  ...
                  <Field
                    label="Role / Position"
                    hint="→ syncs Requisition Name"
                  >
                    <Inp
                      value={form.role}
                      onChange={setRole}
                      placeholder="Operational QA Coordinator / Tester"
                    />
                  </Field>
              ...
              <Card title="Submission Details" icon="📋" delay={100}>
                <G2>
                  <Field label="MSP Name">
                    <Inp
                      value={form.mspName}
                      onChange={set("mspName")}
                      placeholder="KeyMan AB"
                    />
                  </Field>
                  ...
                  <Field label="Requisition ID">
                    <Inp
                      ...
                      onChange={set("requisitionId")}
                      placeholder="HO287890"
                    />
                  </Field>
                  <Field label="Requisition Name" hint="synced with Role">
                    <Inp
                      value={form.requisitionName}
                      onChange={setReqName}
                  ...
                  <div style={{ marginBottom: 13 }}>
                    <Label hint="type gross or net + amount">
                      Candidate Buy Rate
                    </Label>
                    <BuyRateInput
                  ...
                  <Field label="Candidate Submitted">
                    <Inp
                      value={form.candidateSubmitted}
                      onChange={set("candidateSubmitted")}
                      placeholder="14th Apr 2025"
                    />
                  </Field>
                  <Field label="Submission Deadline">
                    <Inp
                      ...
                      placeholder="15th Apr 2025"
                    />
                  </Field>
                  <Field label="Project Duration">
                    <Inp
                      value={form.projectDuration}
                      onChange={set("projectDuration")}
                      placeholder="2026-06-01 to 2027-03-31"
                    />
                  </Field>
                </G2>
              </Card>
                ...
                <G2>
                  ...
                  <Field label="Contact Number" hint="🔒 local only">
                    <Inp
                      ...
                      placeholder="+46(0)7032191048"
                    />
                  </Field>
                  <div style={{ marginBottom: 13 }}>
                    <Label hint="type country or nationality">
                      Nationality
                    </Label>
                    <NationalityInput
                  ...
                  <Field
                    ...
                    hint="auto-fills from nationality"
                  >
                    <Inp
                    ...
                    {form.eligibility && (
                      <div
                        style={{
                          fontSize: 10.5,
                          marginTop: 3,
                          color: isEUNationality(form.nationality)
                            ? "var(--success)"
                            : "#f59e0b",
                        }}
                      >
            ...
            <div>
              ...
              <Card title="AI Sizzling Generator" icon="🤖" glow delay={50}>
                <Alert type="blue">
                  🛡️ <strong>Privacy safe:</strong> Email, phone & DOB replaced
                  with [REDACTED] before being sent to AI.
                </Alert>
                {backendStatus === "down" && (
                  <Alert type="amber">
                ...
                <div style={{ marginTop: 12 }}>
                  <Field
                    label="CV Text"
                    hint="auto-filled on upload (PII stripped) — or paste manually"
                  >
                    <TA
                      value={form.cvText}
                      onChange={set("cvText")}
                ...
                <button
                  ...
                  style={{
                    ...
                    transition: "all 0.2s",
                    opacity: retryCountdown > 0 ? 0.7 : 1,
                  }}
                >
                  {aiLoading ? (
                    <>
                      <SpinEl /> Generating — please wait…
                    </>
                  ) : retryCountdown > 0 ? (
                    `⏳ Retry available in ${retryCountdown}s`
                  ) : (
                    "✨ Generate Sizzling with AI"
                  )}
                </button>
                ...
                {aiSuccess && (
                  <Alert type="green">
                    ✅ AI complete! Review & edit the fields below.
                  </Alert>
                )}
                ...
                <div style={{ marginTop: 16 }}>
                  ...
                  <Field
                    label="Additional Sizzling Bullets"
                    hint="one per line — editable"
                  >
                    <TA
                      value={form.sizzlingExtra}
                      onChange={set("sizzlingExtra")}
                      rows={4}
                      placeholder={
              ...
              <Card title="Candidate Information Bullets" icon="ℹ️" delay={100}>
                <div style={{ marginBottom: 10 }}>
                  {rateInfo.type === "gross" && (
                    ...
                    >
                      💼 Gross rate — last bullet: "Has own Limited company to
                      manage payroll."
                    </div>
                  )}
                  {rateInfo.type === "net" && (
                    ...
                    >
                      💰 Net rate — last bullet: "He/She is willing to work
                      under Avance payroll."
                    </div>
                  )}
                  {form.nationality &&
                    isEUNationality(form.nationality) === true && (
                  ...
                  {form.nationality &&
                    isEUNationality(form.nationality) === false && (
                      ...
                      >
                        🌍 Non-EU — work permit bullet{" "}
                        {form.workAuthorization
                          ? "inserted ✓"
                          : "appears when Work Authorization is filled"}
                      </div>
                    )}
                  {form.candidateName && (
                    <div
                    ...
                    >
                      👤 <strong>{form.candidateName}</strong> — reflected in
                      first bullet
                    </div>
                  )}
                  {candInfoManual && (
                    ...
                    >
                      ✏️ Manually edited — auto-updates paused. Reset to restore
                      auto-mode.
                    </div>
                  )}
                </div>
                <TA
                  ...
                  rows={7}
                  placeholder={buildCandidateInfo(
                    "Candidate Name",
                    null,
                    null,
                    "",
          ...
          <div className="fade-in">
            ...
            >
              <p style={{ color: "var(--text-muted)", fontSize: 12.5 }}>
                📌 Click{" "}
                <strong style={{ color: "var(--text)" }}>Copy Email</strong>{" "}
                then paste into Outlook or Gmail — the table renders correctly.
              </p>
              <button
                onClick={handleCopy}
                style={copied ? successBtn : primaryBtn}
